...
//    private static final int INITIAL_AMMO = 30; // 弹药补充
//
//    // 临时的死亡玩家计时器 ---
...
//        initialState.setHealth(PLAYER_MAX_HEALTH); // 设置满血
//        initialState.setAmmo(INITIAL_AMMO);
//        initialState.setKills(0);
//        initialState.setDeaths(0);
//        initialState.setCurrentAction(PlayerState.PlayerActionState.IDLE); // 设置初始状态
...
//
//                // 在处理任何输入前，先将玩家状态默认置为 IDLE
//                // 后续的移动或开火逻辑会覆盖这个状态
//                attackerState.setCurrentAction(PlayerState.PlayerActionState.IDLE);
//
//                // --- A. 处理移动 ---
//                if (command.getMoveInput() != 0) { // 只有在移动时才设置RUN状态
//                    double newX = attackerState.getX() + command.getMoveInput() * 5.0;
//                    attackerState.setX(newX);
//                    attackerState.setFacingRight(command.getMoveInput() > 0);
//                    // 【修改】设置移动状态
//                    attackerState.setCurrentAction(PlayerState.PlayerActionState.RUN);
//                }
//
//                // --- B. 处理开火 (HitScan) ---
...
//
//                    // 开火前检查弹药
//                    if (attackerState.getAmmo() <= 0) {
//                        // 可以在这里触发一个 "空仓" 事件或声音
//                        continue; // 没有子弹，直接跳过本次开火指令
//                    }
//
//                    // 消耗弹药
//                    attackerState.setAmmo(attackerState.getAmmo() - 1);
//
...
//
//                                // 更新KDA
//                                attackerState.setKills(attackerState.getKills() + 1);       // 攻击者击杀+1
//                                targetState.setDeaths(targetState.getDeaths() + 1);         // 被击杀者死亡+1
//
