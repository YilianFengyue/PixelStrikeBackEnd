...
public class FriendController {
    ...
    @GetMapping("/search")
    public CommonResponse<List<FriendListDTO>> searchUsers(@RequestParam String nickname, ServerWebExchange exchange) {
        // 从请求中获取当前登录用户的 ID
    ...
    public CommonResponse<?> sendFriendRequest(@PathVariable Integer addrId, ServerWebExchange exchange) {
        Integer senderId = exchange.getAttribute("userId");
        return friendService.sendFriendRequest(senderId, addrId);
    }

    // 查看我的好友申请 (GET /friends/requests/pending)
    @GetMapping("/requests/pending")
    public CommonResponse<List<FriendListDTO>> getPendingRequests(ServerWebExchange exchange) {
    ...
    public CommonResponse<?> acceptFriendRequest(@PathVariable Integer senderId, ServerWebExchange exchange) {
        Integer userId = exchange.getAttribute("userId");
    ...
    public CommonResponse<?> rejectFriendRequest(@PathVariable Integer senderId, ServerWebExchange exchange) {
        Integer userId = exchange.getAttribute("userId");
     ...
     */
    @GetMapping
    public CommonResponse<List<FriendListDTO>> getFriendList(ServerWebExchange exchange) {
        Integer userId = exchange.getAttribute("userId");
    ...
    public CommonResponse<FriendDetailDTO> getFriendDetails(@PathVariable Integer friendId, ServerWebExchange exchange) {
        Integer currentUserId = exchange.getAttribute("userId");
