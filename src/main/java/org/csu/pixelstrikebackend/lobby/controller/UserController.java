...
public class UserController {
    ...
    public CommonResponse<?> updateNickname(@RequestParam String newNickname, HttpServletRequest exchange) {
        Integer userId = (Integer) exchange.getAttribute("userId");
        return userService.updateNickname(userId, newNickname);
    }

    @PostMapping("/me/avatar")
    public CommonResponse<?> updateAvatar(@RequestPart("avatar") MultipartFile file, HttpServletRequest exchange) { // 返回类型改为 Mono<CommonResponse<?>>
        Integer userId = (Integer) exchange.getAttribute("userId");
        return userService.updateAvatar(userId, file); // userService 现在会返回一个 Mono
    }
